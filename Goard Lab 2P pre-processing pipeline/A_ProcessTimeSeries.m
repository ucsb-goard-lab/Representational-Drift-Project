...
for i = 1:lengthList
        ...
        for j = 1:length(cfg_file)
            if strfind(cfg_file{j},'repetitionPeriod') > 0
                ...
                if isinf(data.frameRate)
                    data.frameRate = 10; %input('Enter frame rate (Hz): ')
                end
            end
        end
    else
    ...
    frame_F = [];
    max_proj = zeros(data.yPixels,data.xPixels,num_blocks);
    ...
    if strcmp(movie_flag,'Yes')
        avg_frame = 5;   % number of frames to average for movie
